...
# $Id: chklb.pl,v 1.2 2009/03/12 21:36:00 larry Exp $
...
foreach (@con1) {
	...
	next unless $_ =~ /(((\d{1,3}\.){3}(\d{1,3})|([[:xdigit:]]{4}:){7}([[:xdigit:]]{4}))(:\d+)? -> \S+)\s+(\S+)/;
...
foreach ( sort keys %con1 ) {
		...
		if ( $Output && $Output eq "raw") {
			print OUT "$_ ($con1{$_}{TYPE}) : $con1{$_}{COUNT} / $con2{$_}{COUNT}\n";
		} elsif ( $Output && $Output eq "full") {
			print OUT "$con1{$_}{FULL}      ====> $con1{$_}{COUNT}\n$con2{$_}{FULL}      ====> $con2{$_}{COUNT}\n\n\n";
		} else {
			write OUT;
		}
		$fails++;
